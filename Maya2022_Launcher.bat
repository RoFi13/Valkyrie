...
set MAYA=C:\Program Files\Autodesk\Maya2022\
...
for %%I in ("%~dp0..") do (
    set "parent_dir=%%~fI"
    set "MAYA_PROJECT=%%~fI\Maya"
)

rem \---- Start Maya - Leaves a cmd prompt window open. Feel free to close.
for /f "tokens=2*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Autodesk\Maya\2022\Setup\InstallPath" /v MAYA_INSTALL_LOCATION') do set "MAYA_LOCATION=%%~b"
